...
mod macros {
    ...
    macro_rules! once {
        ...
        ( $variable:ident, $f:ident, $ifnothing:expr, $error:expr ) => {
            if $variable.$f() {
                return Err($error);
            } else {
                $variable = $ifnothing;
...
pub fn generate_file_map(input_path : &str) -> Result<FileMap, ParseError> {
    Ok(FileMap {})
}
...
fn parse_lit_file<'a>(lines: Vec<LitBlock<'a>>) -> ParseResult<(LitFile, CssSettings)> {
    let mut title = None;
